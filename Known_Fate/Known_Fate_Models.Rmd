...
for(t in 2:42){
surv.prob[,t] = plogis(.8 + .15*fake.data$sex -.19*(as.numeric(as.factor(fake.data$color))-1) + .85*ran_sow[fake.data$Sow]+.05*t)
}
...
We end up with 1112 time periods, though obviously we don't have data for most of those. 
...
Note that below I've written beta.sex[1] as deterministic, not drawn from a distribution. Why? The reason is that if both beta.sex values come from distributions and beta0 is also unknown, the model will be overparameterized. No good! This is something to keep in mind when you have categorical fixed effects and intercepts together. A good rule of thumb is to always set one group of the fixed effects to 0 to avoid this issue. Or just get rid of the intercept. 

(Special thanks to Dr. Frances Buderman for catching this error from a previous version of this tutorial.)
...
jd <- list(n.ind= nrow(neos), y = surv.caps, first = first, 
           ...
           n.sow = n.sow) 
ji <- function(){list(beta0 = .5, beta.sex = c(NA, runif(1)), beta.year = runif(3))}
...
So what do these results tell us? Firstly, if you were born in year 2, your survival chances are probably lower than piglets born in other years (based on the median beta.year estimates). Secondly there is some evidence of lower survival for male piglets (the median of beta.sex[2] is less than 0) but not much of a difference between sexes and the CI crosses 0.
...
n.inits <- list(beta0 = runif(1), beta.age = runif(1), beta.mass = runif(1), 
                beta.Sow = runif(n.sow), beta.sex = c(NA, runif(1)), sigmayear = runif(1),
                sigmaSow = runif(1))
```
...
```{r}
mcmcpigs <- configureMCMC(preppigs, monitors = n.params, print = T, enableWAIC = TRUE)
...
```{r}
tail(as.mcmc(as.matrix(Comppigs$mvSamples))) #many variables available to look at
```
...
nimblepigs.Sow <- clusterEvalQ(cl = cl, {
 preppigs$on <- c(1,0,0,0,1)
  ...
  Comppigs <- compileNimble(pigsMCMC, project = preppigs)
  Comppigs$run(niter = 1000, nburnin = 500)
  return(list(outs = as.mcmc(as.matrix(Comppigs$mvSamples)), waic = Comppigs$getWAIC()))
})
```
...
No surprise, the results are basically the same as we got from JAGS. (Also, yes, we needed to run these chains for longer.) 
...
WAIC_nim[i,2] <-  mean(unlist(nim.models[[i]],F)[[2]]$WAIC,
                       unlist(nim.models[[i]],F)[[4]]$WAIC,
                       unlist(nim.models[[i]],F)[[6]]$WAIC)
}
