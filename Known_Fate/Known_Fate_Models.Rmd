...
So first, let's build our big model with all the covariates in it. In NIMBLE we can ask it to calculate WAIC for us, but with the latest NIMBLE update (v 0.10.1), the built in function requires that we monitor "y". That's not always ideal, so I'm going to just do the calculations myself. 

A note about calculating WAIC in NIMBLE - you can use R functions!! logdensity.binom only exists in JAGS, so instead we'll use the function "dbinom" from R. 
...
for (i in 1:n.ind){
  ...
  logit(phi[i,t]) <- beta0 + beta.Sow[sow[i]] + 
    ...
    mu[i,t] <- phi[i,t]*y[i,t-1]
    y[i,t] ~dbern(mu[i,t])
    loglike[i,t] <- dbinom(y[i,t], 1, mu[i,t], log = T) #for WAIC
  }
}
beta0 ~ dunif(-6,6)
beta.age ~ dnorm(0,.5) #constrain to avoid difficulties with convergence
...
```
First up we'll notice this big scary notification that "Model includes deterministic assignment using '<-' of the result of a density ('d') or simulation ('r') calculation. This is likely not what you intended in:". Except, this is what we intended. So we will ignore it. 

We also seem to be missing some values that we'll want to initialize. First we'll have NIMBLE find values for beta.year, then we'll have it follow through with those values to initialize phi and mu. Note that y will always have some NA's - those are all the times the piglets weren't born or we weren't following them around. 
...
Onwards to the other parts of the model. Note that if we wanted to use the built in WAIC function, we would need to add "y" to our params list and add "enableWAIC = T".
```{r, eval = F}
n.params <- c(n.params[-"loglike"], "y")
mcmcpigs <- configureMCMC(preppigs, monitors = n.params, print = T, enableWAIC = T)
```

Instead, we will just calculate WAIC ourselves at the end. 

```{r}
mcmcpigs <- configureMCMC(preppigs, monitors = n.params, print = T)
...
```{r}
as.mcmc(as.matrix(Comppigs$mvSamples))[1:5, 29:32]
```
Looks fairly reasonable! It might seem like beta.year is bouncing around a lot but remember because we "turned off" all the covariates, the only relevant one is beta0. 

If we had used the internal WAIC feature, we could use this method to extract:
```{r, eval = F, echo = T}
Comppigs$calculateWAIC()
```

But we did not, so we'll have to use the same method we used in JAGS. We can wait to calculate that till we have a full run of the model.
...
```{r}
sexyear.outputs <- mcmc.list(nimblepigs.sexyear)
...
We can use similar code to what we did with JAGS to collect our model WAICs.
```{r}
