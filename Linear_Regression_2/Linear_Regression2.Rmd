...
E(weight) <- interceptA*(1 if species A, 0 if species B) + interceptB*(1 if species B, 0 if species A) + age X something + leg X something2 + distance X something3
...
inits <- list(beta0 = runif(2, -10, 10), beta1 = runif(1,-10,10), 
              beta2 = runif(1,-10,10), beta3 = runif(1,-10, 10),
              sd = runif(1, .001, 100))
```
...
```{r tables, eval = F}
data.frame(Mods = 1:3, DICS = c(sum(Dic_1$deviance),
  sum(Dic_2$deviance), sum(Dic_3$deviance)), 
  WAICS = c(WAIC, WAIC2, WAIC3))
```
```{r pretty out, echo = F}
...
```{r coef post}
coef.post <- as.matrix(Frog.mod3$mcmc[[1]][9000:10000,
